...
function initDrag() {
  ...
  grid.addEventListener('dragstart', e => {
    ...
    dragSrcId = parseInt(c.dataset.id); c.style.opacity = '0.5';
    e.dataTransfer.effectAllowed = 'move';
  });
  grid.addEventListener('dragover', e => {
    e.preventDefault();
    document.querySelectorAll('.drag-over').forEach(el => el.classList.remove('drag-over'));
    const c = e.target.closest('.staff-card[draggable]'); if (c) c.classList.add('drag-over');
  });
  grid.addEventListener('drop', e => {
    e.preventDefault();
    document.querySelectorAll('.drag-over').forEach(el => el.classList.remove('drag-over'));
    const c = e.target.closest('.staff-card[draggable]'); if (!c) return;
    const tId = parseInt(c.dataset.id);
    if (dragSrcId === null || dragSrcId === tId) return;
    const src = W.find(x => x.id===dragSrcId), tgt = W.find(x => x.id===tId);
  ...
  grid.addEventListener('dragend', () => {
    dragSrcId = null;
    document.querySelectorAll('.staff-card').forEach(el => { el.classList.remove('drag-over'); el.style.opacity = ''; });
  });
}
